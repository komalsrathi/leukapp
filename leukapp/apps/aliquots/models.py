...
class Aliquot(LeukappModel):
    ...
    """
    ID used by the Institution or Scientist to track the :class:`.Aliquot`.
    ...
    ``[5]`` of the **leukid** (e.g. ``1``):

    5.  ``1`` indicates that the current :class:`Aliquot` is the first
        **tube** used from the parent class
