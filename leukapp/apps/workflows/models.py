...
class Workflow(LeukappModel):
    ...
    Each :class:`.Workflow` can be linked to multiple
    ...
    :meth:`_get_projects_from_string` to link projects and workflows::
    ...
    For example, the internal code ``1-1A-S100-x10-CMO`` will refer to:

    * ``1``: count of ``Workflows`` per ``Extraction``, see:
      :attr:`~leukapp.apps.extractions.models.Extraction.workflows_count`.
    * ``1A``: :attr:`technology`/:attr:`technology_type` internal code based
      on the custom dictionary :data:`~.constants.INT_ID_TECHNOLOGY`.
    * ``S100``: Internal code for the :attr:`read_length`/:attr:`read_type`
      combination. see :data:`~.constants.INT_ID_READ_LENGTH` and
      :data:`~.constants.INT_ID_READ_TYPE`.
    * ``X10``: :attr:`sequencing_platform` internal code. See:
      :data:`~.constants.INT_ID_PLATFORM`.
    * ``CMO``: :attr:`sequencing_center` internal code. See:
